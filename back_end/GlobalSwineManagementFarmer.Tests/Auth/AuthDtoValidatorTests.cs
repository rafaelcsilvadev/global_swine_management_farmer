...
public class AuthDtoValidatorTests
{
    ...
    [Fact]
    public void Should_Have_Error_When_Email_Exceeds_255_Characters()
    {
        var model = new AuthDto(new string('a', 247) + "@test.com", "Password123!", Guid.NewGuid());
        var result = _validator.TestValidate(model);
        result.ShouldHaveValidationErrorFor(x => x.Email);
    }
    ...
    {
        var model = new AuthDto("test@test.com", "Aa1!" + new string('x', 97), Guid.NewGuid());
        var result = _validator.TestValidate(model);
        result.ShouldHaveValidationErrorFor(x => x.Password);
    }
    ...
    public void Should_Handle_SQL_Injection_Patterns_In_Password(string injectionPassword)
    {
        ...
        _ = _validator.TestValidate(model);
    }
